...
| 2026-07-11 | Live demo v1 private serving bundle | Trained train-only Ridge, XGBoost, and MLP prediction bundles for a future interactive demo, using an internal 90/10 partition of reconstructed train rows for XGBoost/MLP selection. | Private bundle only; no reconstructed official validation/test labels read, no new benchmark result, API, UI, commit, push, or deployment. Exact procedure and hashes: `docs/live_demo_v1_process.md`. |
| 2026-07-11 | Live demo v1 deterministic reload correction | Set the safetensors MLP serving loader to `eval()` plus `torch.inference_mode()` before predicting, then regenerated its private self-test through the fixed reload path. | Two consecutive reload predictions were identical and matched private self-test SHA-256 `c93b700…d958`; no retraining or label read; no API/UI/deployment change. |
| 2026-07-12 | Live demo v1 local private API | Added a loopback-only FastAPI wrapper with fixed `/healthz`, `/v1/model-card`, and single-SMILES `/v1/predict` endpoints; loaded the verified bundle and audited MIST snapshot once at startup. | Actual endpoint test passed, including invalid SMILES and repeat-stable MLP predictions. No request-selected model/path/device, request logging, upload, commit, push, public deployment, or weight redistribution. |
